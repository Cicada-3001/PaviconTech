...
import { recoverPassword } from '../libs/password';
...
function Page() {
    ...
    async function onSubmit(data){
      try{
        //console.log(data)
        setLoading(true)
        const response = recoverPassword(data)
        ...
        const { response } = error
          ...
          if(response.status === 404){
              ...
              toast.error("Ops something went wrong  sana!")
            }
      }
    }
    ...
    return (
      !isCodeSent ? <div className="font-[sans-serif] text-[#333] bg-white">
      <div className="grid lg:grid-cols-4 md:grid-cols-3 items-center">
        <form  onSubmit={handleSubmit(onSubmit)} className="lg:col-span-3 md:col-span-2 max-w-lg w-full p-6 mx-auto" >
          <div className="mb-16">
            <h3 className="text-4xl font-extrabold">Recover Your Password</h3>
            <p className="text-sm mt-6">Please enter the  email your used to sign up your account with.</p>
          </div>
          ...
          <div className="mt-10">
          {
            loading ? (
              <div className='mt-10'> 
                <button disabled="" type="button" className="w-full shadow-xl py-2.5 px-4 text-sm font-semibold rounded  bg-blue-600 text-white  hover:bg-blue-700 focus:outline-none border border-slate-50 hover:border-0">
                  ...
                  Sending Reset Code, Please wait ...
                </button>
              </div>
            ):(<div className="mt-10">
